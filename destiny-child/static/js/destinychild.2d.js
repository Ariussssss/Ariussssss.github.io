...
function initModel(pathDir) {
  if (pathDir == "kr") {
    dir = "/destiny-child/static/Korean/";
  } else dir = "/destiny-child/static/Global/";
...
function init(dir, canvas) {
  ...
  loadBytes(
    getPath(dir, modelJson.motions.idle[0].file),
    "arraybuffer",
    function (buf) {
  ...
  if (modelJson.motions.attack) {
    loadBytes(
      getPath(dir, modelJson.motions.attack[0].file),
      "arraybuffer",
      function (buf) {
    ...
    loadBytes(
      getPath(dir, modelJson.motions.maxtouch[0].file),
      "arraybuffer",
      function (buf) {
        motionClick = new Live2DMotion.loadMotion(buf);
        // remove fade in/out delay to make it smooth
