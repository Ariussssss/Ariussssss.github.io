...
function GView() {
  document.getElementById("note").innerHTML =
    "Viewing Global's censored version of the game";
  diffView = "g";
  updateViewer();
}
...
function KView() {
  document.getElementById("note").innerHTML =
    "Viewing Korean's uncensored version of the game";
  diffView = "k";
  updateViewer();
}
...
function updateViewer(id) {
  if (id && typeof id == "string") {
    ...
    if (window.history.replaceState) {
      ...
      window.history.replaceState(id, id, `?value=${id}`);
    }
  }
  ...
  if (diffView == "k") {
    viewer.src =
      ...
      "&mY=" +
      mY;
    code = viewer.outerHTML.replace(
      "viewerk.html",
      document.location.toString() + "viewerk.html"
    );
    ...
    code = viewer.outerHTML.replace(
      "viewer.html",
      document.location.toString() + "viewer.html"
    );
    download =
      globalURL + "hd_mobile/asset/character/" + selectedModel + ".pck";
  }

  if (diffView == "k") {
    $("#Uncensored").addClass(
      "ui-button ui-corner-all ui-widget ui-button-disabled ui-state-disabled"
    );
...
function loadChilds(callback) {
  $.getJSON("/destiny-child/static/json/childs.json", function (_childs) {
    childs = _childs;
    callback();
  });
}
...
function loadAssets(callback) {
  $.getJSON("/destiny-child/static/json/assets.json", function (data) {
  ...
  loadChilds(function () {
    loadAssets(function () {
      ...
      if (modelIndex > -1) setSelectedIndex(modelIndex);
        ...
        const params = location.search.split("?").pop();
        const { value = null } = Object.fromEntries(
          params
            .split("&")
            .filter(Boolean)
            .map((e) => e.split("="))
        ...
        if (value) {
          $("select").combobox("instance").input.val(getLabel(value));
        }
        updateViewer(value);
      }
      $("#loading").hide();
